...
if [ "$2" == "--setup" ] || [ "$2" == "--jar" ]; then
    ...
    echo "Upstream directory does not exist. Did you forget to run 'unicorn setup'?"
    exit 1
fi

# Apply UnicornSpigot
(
    applyPatch "base/Paper/PaperSpigot-API" UnicornSpigot-API HEAD patches/api &&
    applyPatch "base/Paper/PaperSpigot-Server" UnicornSpigot-Server HEAD patches/server
    cd "$basedir"
) || (
    echo "Failed to apply Paper Patches"
    exit 1
