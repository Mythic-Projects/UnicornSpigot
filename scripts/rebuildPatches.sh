...
savePatches "$workdir/Paper/PaperSpigot-API" "UnicornSpigot-API" "patches/api"
if [ -f "$basedir/UnicornSpigot-API/.git/patch-apply-failed" ]; then
    echo "$(color 1 31)[[[ WARNING ]]] $(color 1 33)- Not saving UnicornSpigot-Server as it appears UnicornSpigot-API did not apply clean.$(colorend)"
    echo "$(color 1 33)If this is a mistake, delete $(color 1 34)UnicornSpigot-API/.git/patch-apply-failed$(color 1 33) and run rebuild again.$(colorend)"
    echo "$(color 1 33)Otherwise, rerun ./unicorn patch to have a clean UnicornSpigot-API apply so the latest UnicornSpigot-Server can build.$(colorend)"
else
    savePatches "$workdir/Paper/PaperSpigot-Server" "UnicornSpigot-Server" "patches/server"
fi
) || exit 1
